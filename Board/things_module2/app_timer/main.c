...
static app_timer_id_t   app_timer_1;

void app_error_handler(uint32_t error_code, uint32_t line_num, const uint8_t * p_file_name)
{
}

static void TimerTimeout( void * p_context ){
    nrf_gpio_pin_toggle(GPIO_TOGGLE_TICK_EVENT);
    app_timer_start(app_timer_1,APP_TIMER_TICKS(1000,0),NULL);
}
...
int main(void)
{
    ...
    uint32_t err_code = app_timer_create(&app_timer_1,
                                APP_TIMER_MODE_SINGLE_SHOT,
                                TimerTimeout);    
    APP_ERROR_CHECK(err_code);

    app_timer_start(app_timer_1,APP_TIMER_TICKS(10000,0),NULL);
    while (true)
    {
        // Do nothing.
    }
